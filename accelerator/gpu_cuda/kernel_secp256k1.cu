...
{
    init_gpu_secp256k1(batch_size);
    init_gpu_keccak(batch_size);
}
...
{
    unsigned char* digests = run_keccak(batch_size, messages, message_lengths);
    return run_secp256k1(batch_size, pkeys, signatures, digests);
}
